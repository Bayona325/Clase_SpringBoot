const URL_API = "http://localhost:9080";
const myHeaders = new Headers({
    "Content-Type": "application/json"
});
const getContact = async() => {
    try {
        const respuesta = await fetch(`${URL_API}/contactos/crear`);
...
const postContact = async (datos) => {
    try {
        return await fetch(`${URL_API}/contactos/crear`, {
            method: "POST",
            headers: myHeaders,
...
const patchContact = async (datos,id) =>{
    ...
    try {
        return await fetch(`${URL_API}/contactos/crear/${id}`, {
            method: "PATCH",
            headers: myHeaders,
            body: JSON.stringify(datos)
        });
